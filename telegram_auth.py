...
1. Visit: https://api.iiflcapital.com/
...
⏰ AUTH_CODE expires in 15 minutes
    ...
    async def auth_code_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """Handle AUTH_CODE submission."""
        auth_code = update.message.text.strip()
        
        if not auth_code or len(auth_code) < 5:
            await update.message.reply_text("❌ Invalid AUTH_CODE format. Please try again.")
            return WAITING_FOR_AUTH_CODE
    ...
    async def confirm_auth_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # Import here to use the auth_code
            from config import config
            from broker_iifl import broker
            import os
            
            # Set auth code in environment
...
Please try again with a fresh AUTH_CODE.
            ...
            # Save auth code to file for persistence
            if login_result:
                with open('.iifl_auth', 'w') as f:
                    f.write(self.auth_code)
                logger.info("AUTH_CODE saved for session persistence")
